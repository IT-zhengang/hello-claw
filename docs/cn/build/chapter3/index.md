...
| 能力层 | `.codex/skills/` | 技能卡片 | 告诉 Agent 遇到某类任务时，该去哪读哪份 `SKILL.md` |
...
| `.codex/skills/` 里的技能 | 先注入紧凑索引，真正需要时再读 `SKILL.md` | 减少常驻上下文开销，避免几十个技能全文一起挤爆窗口 |
...
系统会先把当前可用技能整理成紧凑列表注入 prompt，模型判断需要某个技能时，再去读那份 `SKILL.md`。在当前这类 Codex 风格项目里，项目级技能更推荐放在 `.codex/skills/`，旧根目录 `skills/` 可以视作历史兼容写法。
...
| 某类低频但专业的流程 | `.codex/skills/某技能/SKILL.md` |
