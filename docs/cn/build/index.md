...
- 所以当前第三板块会先落到已经发布的第 13 章，把它当成"从理解系统走向第一次动手扩展"的起点。
...
所以第 13 章虽然编号靠后，但它在阅读体验上更像第三板块的**第一个实操落点**。内容也会从描述到代码逐步展开：
...
如果你已经有一个具体的定制目标（"我想让它接入公司的XX系统"），直接跳到第13章，再配合附录 D。边读边做，在实践中有针对性地回头查原理。
